/*
  ...
  19.04.2026 V05 Implemented Purple, Yellow and Mint as delayless flashing colors
...
//#define ONBOARD_LED_PIN 13 // H2 // this is a small blue LED
#define ONBOARD_LED_PIN 15 // C6 // this is a small blue LED
...
uint8_t RGB_LED_FLASH_DURATION_MILLIS = 50;
...
bool setYellowLedFlash = false;
bool setPurpleLedFlash = false;
bool setMintLedFlash = false;
...
bool isPurpleLedOn = false;
bool isMintLedOn = false;

long lastRedLedFlashMillis = 0;
long lastGreenLedFlashMillis = 0;
long lastBlueLedFlashMillis = 0;
long lastWhiteLedFlashMillis = 0;
long lastYellowLedFlashMillis = 0;
long lastPurpleLedFlashMillis = 0;
long lastMintLedFlashMillis = 0;
...
void rgbLedFlashLoop() {
  ...
  if (isRedLedOn) {
    if ((millis() - lastRedLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedRed(0);
      isRedLedOn = false;
    } 
  }
  ...
  if (isGreenLedOn) {
    if ((millis() - lastGreenLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedGreen(0);
      isGreenLedOn = false;
    } 
  }
  ...
  if (isBlueLedOn) {
    if ((millis() - lastBlueLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedBlue(0);
      isBlueLedOn = false;
    } 
  }
  ...
  if (isWhiteLedOn) {
    if ((millis() - lastWhiteLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedWhite(0);
      isWhiteLedOn = false;
    } 
  }

  if (setYellowLedFlash) {
    setYellowLedFlash = false;
    isYellowLedOn = true;
    setRgbLedYellow(ONBOARD_RGB_LED_DEFAULT_BRIGHTNESS);
    lastYellowLedFlashMillis = millis();
  }

  if (isYellowLedOn) {
    if ((millis() - lastYellowLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedYellow(0);
      isYellowLedOn = false;
    } 
  }

  if (setPurpleLedFlash) {
    setPurpleLedFlash = false;
    isPurpleLedOn = true;
    setRgbLedPurple(ONBOARD_RGB_LED_DEFAULT_BRIGHTNESS);
    lastPurpleLedFlashMillis = millis();
  }

  if (isPurpleLedOn) {
    if ((millis() - lastPurpleLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedPurple(0);
      isPurpleLedOn = false;
    } 
  }
  ...
  if (setMintLedFlash) {
    setMintLedFlash = false;
    isMintLedOn = true;
    setRgbLedMint(ONBOARD_RGB_LED_DEFAULT_BRIGHTNESS);
    lastMintLedFlashMillis = millis();
  }

  if (isMintLedOn) {
    if ((millis() - lastMintLedFlashMillis) > RGB_LED_FLASH_DURATION_MILLIS) {
      setRgbLedMint(0);
      isMintLedOn = false;
    } 
  }
