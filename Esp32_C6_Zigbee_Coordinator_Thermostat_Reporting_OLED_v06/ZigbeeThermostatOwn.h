...
/*
  Version management
27.03.2026 V01 This is a modified Thermostat.h/.cpp file version. Renamed to 
               ThermostatOwn.h/.cpp.
               Added the Power Config cluster to the binding and a callback
               when a new battery reading is made.
*/
...
class ZigbeeThermostatOwn : public ZigbeeEP {
public:
  ZigbeeThermostatOwn(uint8_t endpoint);
  ~ZigbeeThermostatOwn() {}
  ...
  void setTemperatureHumidityReporting(uint8_t endpoint, uint16_t short_addr, uint16_t min_interval, uint16_t max_interval, float delta, float delta_hum);

  // power management reportings
  void onBatteryVoltageReceiveWithSource(void (*callback)(uint8_t, uint8_t, esp_zb_zcl_addr_t)) {
    _on_battery_voltage_receive_with_source = callback;
  }

  void onBatteryPercentReceiveWithSource(void (*callback)(uint8_t, uint8_t, esp_zb_zcl_addr_t)) {
    _on_battery_percent_receive_with_source = callback;
  }

private:
  // save instance of the class in order to use it in static functions
  static ZigbeeThermostatOwn *_instance;
  ...
  void (*_on_battery_voltage_receive_with_source)(uint8_t, uint8_t, esp_zb_zcl_addr_t);
  void (*_on_battery_percent_receive_with_source)(uint8_t, uint8_t, esp_zb_zcl_addr_t);
