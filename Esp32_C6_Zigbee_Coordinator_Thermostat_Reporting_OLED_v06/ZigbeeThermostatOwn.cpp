...
ZigbeeThermostatOwn *ZigbeeThermostatOwn::_instance = nullptr;

ZigbeeThermostatOwn::ZigbeeThermostatOwn(uint8_t endpoint)
  : ZigbeeEP(endpoint) {
  ...
  _on_battery_voltage_receive_with_source = nullptr;
  _on_battery_percent_receive_with_source = nullptr;
...
void ZigbeeThermostatOwn::bindCb(esp_zb_zdp_status_t zdo_status, void *user_ctx) {
  ZigbeeThermostatOwn *instance = static_cast<ZigbeeThermostatOwn *>(user_ctx);
...
void ZigbeeThermostatOwn::bindCbWrapper(esp_zb_zdp_status_t zdo_status, void *user_ctx) {
  ZigbeeThermostatOwn *instance = static_cast<ZigbeeThermostatOwn *>(user_ctx);
  if (instance) {
...
void ZigbeeThermostatOwn::findCbWrapper(esp_zb_zdp_status_t zdo_status, uint16_t addr, uint8_t endpoint, void *user_ctx) {
  ZigbeeThermostatOwn *instance = static_cast<ZigbeeThermostatOwn *>(user_ctx);
  if (instance) {
...
void ZigbeeThermostatOwn::findCb(esp_zb_zdp_status_t zdo_status, uint16_t addr, uint8_t endpoint, void *user_ctx) {
  ZigbeeThermostatOwn *instance = static_cast<ZigbeeThermostatOwn *>(user_ctx);
  if (zdo_status == ESP_ZB_ZDP_STATUS_SUCCESS) {
    ...
    esp_zb_zdo_device_bind_req(&bind_req, ZigbeeThermostatOwn::bindCbWrapper, this);

    // add the Power Config Cluster
    // https://github.com/espressif/esp-zigbee-sdk/blob/3d86dd0c41923c68f5fa0aae519c3fe157e8641c/components/esp-zigbee-lib/include/zcl/esp_zigbee_zcl_common.h#L99
    // ESP_ZB_ZCL_CLUSTER_ID_POWER_CONFIG = 0x0001U, //!< Power configuration cluster identifier.
    // this part is the battery cluster
    //
    bind_req.cluster_id = ESP_ZB_ZCL_CLUSTER_ID_POWER_CONFIG;
    log_v("Try to bind ESP_ZB_ZCL_CLUSTER_ID_POWER_CONFIG");
    esp_zb_zdo_device_bind_req(&bind_req, ZigbeeThermostatOwn::bindCbWrapper, NULL);
...
void ZigbeeThermostatOwn::findEndpoint(esp_zb_zdo_match_desc_req_param_t *param) {
  ...
  esp_zb_zdo_match_cluster(param, ZigbeeThermostatOwn::findCbWrapper, this);
}

void ZigbeeThermostatOwn::zbAttributeRead(uint16_t cluster_id, const esp_zb_zcl_attribute_t *attribute, uint8_t src_endpoint, esp_zb_zcl_addr_t src_address) {
  ...
  if (cluster_id == ESP_ZB_ZCL_CLUSTER_ID_POWER_CONFIG) {
    char buf[60];
    uint8_t val = attribute->data.value ? *(uint8_t *)attribute->data.value : 0;

    sprintf(buf, "power config cluster val:%d id: %d dataType: %d", val, attribute->id, attribute->data.type);
    log_i("%s", buf);

    sprintf(buf, "power config cluster val:%2X id: %d dataType: %2X", val, attribute->id, attribute->data.type);
    log_i("%s", buf);
    // https://github.com/espressif/esp-zigbee-sdk/blob/3d86dd0c41923c68f5fa0aae519c3fe157e8641c/components/esp-zigbee-lib/include/zcl/esp_zigbee_zcl_power_config.h
    ...
    if (attribute->id == ESP_ZB_ZCL_ATTR_POWER_CONFIG_BATTERY_VOLTAGE_ID && attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_U8) {
      uint8_t batVolt = attribute->data.value ? *(uint8_t *)attribute->data.value : 0;
      // the reporting is battery voltage * 10
      if (_on_battery_voltage_receive_with_source) {
        _on_battery_voltage_receive_with_source(batVolt, src_endpoint, src_address);
      }
    }
    // https://github.com/espressif/esp-zigbee-sdk/blob/3d86dd0c41923c68f5fa0aae519c3fe157e8641c/components/esp-zigbee-lib/include/zcl/esp_zigbee_zcl_power_config.h
    ...
    if (attribute->id == ESP_ZB_ZCL_ATTR_POWER_CONFIG_BATTERY_PERCENTAGE_REMAINING_ID && attribute->data.type == ESP_ZB_ZCL_ATTR_TYPE_U8) {
      uint8_t batPerc = attribute->data.value ? *(uint8_t *)attribute->data.value : 0;
      // the reporting is battery percent * 2
      if (_on_battery_percent_receive_with_source) {
        _on_battery_percent_receive_with_source(batPerc, src_endpoint, src_address);
      }
    }
  }
}
...
void ZigbeeThermostatOwn::getTemperature() {
...
void ZigbeeThermostatOwn::getTemperature(uint16_t group_addr) {
...
void ZigbeeThermostatOwn::getTemperature(uint8_t endpoint, uint16_t short_addr) {
...
void ZigbeeThermostatOwn::getTemperature(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
...
void ZigbeeThermostatOwn::getTemperatureSettings() {
  /* Send "read attributes" command to all bound sensors */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getTemperatureSettings(uint16_t group_addr) {
  /* Send "read attributes" command to the group */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getTemperatureSettings(uint8_t endpoint, uint16_t short_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::getTemperatureSettings(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::setTemperatureReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
...
void ZigbeeThermostatOwn::setTemperatureReporting(uint16_t group_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
...
void ZigbeeThermostatOwn::setTemperatureReporting(uint8_t endpoint, uint16_t short_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
...
void ZigbeeThermostatOwn::setTemperatureReporting(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
  /* Send "configure report attribute" command to specific endpoint */
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
...
void ZigbeeThermostatOwn::getHumidity() {
  /* Send "read attributes" command to all bound sensors */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getHumidity(uint16_t group_addr) {
  /* Send "read attributes" command to the group */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getHumidity(uint8_t endpoint, uint16_t short_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::getHumidity(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::getHumiditySettings() {
  /* Send "read attributes" command to all bound sensors */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getHumiditySettings(uint16_t group_addr) {
  /* Send "read attributes" command to the group */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::getHumiditySettings(uint8_t endpoint, uint16_t short_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::getHumiditySettings(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr) {
  /* Send "read attributes" command to specific endpoint */
  esp_zb_zcl_read_attr_cmd_t read_req;
  memset(&read_req, 0, sizeof(read_req));
  read_req.address_mode = ESP_ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::setHumidityReporting(uint16_t min_interval, uint16_t max_interval, float delta) {
  /* Send "configure report attribute" command to all bound sensors */
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
  report_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_DST_ADDR_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::setHumidityReporting(uint16_t group_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
  /* Send "configure report attribute" command to the group */
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
  report_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_16_GROUP_ENDP_NOT_PRESENT;
...
void ZigbeeThermostatOwn::setHumidityReporting(uint8_t endpoint, uint16_t short_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
  /* Send "configure report attribute" command to specific endpoint */
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
  report_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
...
void ZigbeeThermostatOwn::setHumidityReporting(uint8_t endpoint, esp_zb_ieee_addr_t ieee_addr, uint16_t min_interval, uint16_t max_interval, float delta) {
  /* Send "configure report attribute" command to specific endpoint */
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
  report_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_64_ENDP_PRESENT;
...
// This is a combined reporting request for temperature and humidity
void ZigbeeThermostatOwn::setTemperatureHumidityReporting(uint8_t endpoint, uint16_t short_addr, uint16_t min_interval, uint16_t max_interval, float delta, float delta_hum) {
  /* Send "configure report attribute" command to specific endpoint */
  // temperature
  esp_zb_zcl_config_report_cmd_t report_cmd;
  memset(&report_cmd, 0, sizeof(report_cmd));
  ...
  // humidity
  esp_zb_zcl_config_report_cmd_t hum_report_cmd;
  memset(&hum_report_cmd, 0, sizeof(hum_report_cmd));
  hum_report_cmd.address_mode = ESP_ZB_APS_ADDR_MODE_16_ENDP_PRESENT;
  hum_report_cmd.zcl_basic_cmd.src_endpoint = _endpoint;
  hum_report_cmd.zcl_basic_cmd.dst_endpoint = endpoint;
  hum_report_cmd.zcl_basic_cmd.dst_addr_u.addr_short = short_addr;
  hum_report_cmd.clusterID = ESP_ZB_ZCL_CLUSTER_ID_REL_HUMIDITY_MEASUREMENT;

  int16_t hum_report_change = (int16_t)delta_hum * 100;
  esp_zb_zcl_config_report_record_t hum_records[] = {
    {
      .direction = ESP_ZB_ZCL_REPORT_DIRECTION_SEND,
      .attributeID = ESP_ZB_ZCL_ATTR_REL_HUMIDITY_MEASUREMENT_VALUE_ID,
      .attrType = ESP_ZB_ZCL_ATTR_TYPE_U16,// temp is s16, hum = u16 ESP_ZB_ZCL_ATTR_TYPE_S16,
      .min_interval = min_interval,
      .max_interval = max_interval,
      .reportable_change = (void *)&hum_report_change,
    },
  };
  hum_report_cmd.record_number = ZB_ARRAY_LENGHT(hum_records);
  hum_report_cmd.record_field = hum_records;

  log_i("Sending 'configure temperature and humidity reporting' command to endpoint %u, address 0x%x", endpoint, short_addr);
  esp_zb_lock_acquire(portMAX_DELAY);
  //esp_zb_zcl_config_report_cmd_req(&report_cmd);
  esp_err_t err = esp_zb_zcl_config_report_cmd_req(&report_cmd);
  Serial.printf("err temperature: %d\n", err);
  
  // short pause to avoid collisions in stack
  delay(100); 
  esp_zb_zcl_config_report_cmd_req(&hum_report_cmd);
